...
def login_hacker():
    ...
    cursor.execute("SELECT * FROM hackers WHERE alias=? AND password=?",[alias_name,user_password,])
    hacker_login = cursor.fetchone()
    print(hacker_login)
    if(hacker_login != None):
        print('sucess login')
        hacker_choices(hacker_login)
    else:
...
def hacker_choices(hacker_login):
    ...
    while True:
        if(hacker_choice == '1'):
            ...
            exploit = input()
            cursor.execute("INSERT INTO exploits(content,user_id) VALUES (?,?)",[exploit,hacker_login[0]])
            conn.commit()
            print('exploit created')
        elif(hacker_choice == '2'):
            cursor.execute("SELECT * FROM exploits WHERE user_id=?",[hacker_login[0]])
